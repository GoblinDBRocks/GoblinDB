 ...
 * @returns {arrat} Parsed db.
 */
function restoreFn(db) {
	return db.slice().map(function(amb) {
		if (amb.action) {
			amb = Object.assign({}, amb, {action: JSONfn.parse(amb.action)});
		}
...
function compileFn(db) {
	return db.slice().map(function(amb) {
		if (amb.action) {
			amb = Object.assign({}, amb, {action: JSONfn.stringify(amb.action)});
		}
...
function run(id, parameter, callback) {
	...
	if (index > -1) {
		if (typeof(goblin.ambush[index].action) !== 'function') {
			console.log(goblin.ambush[index].action)
		}
...
function ambushChanged(details) {
