...
function init() {
	Storage.read(goblin.config.files.ambush).then(function(db, success) {
		goblin.ambush = eval(db);

		if (!success) {
			goblin.ambush = [];
			Storage.save(goblin.config.files.ambush, goblin.ambush);
		}
	});
}
...
function ambushChanged() {
    ...
    Storage.save(goblin.config.files.ambush, []).then(function(saved) {
        if (!saved) {
            logger('AMBUSH_CLEAN_FS');
        }

        // And now, save the real ambush functions
        Storage.save(goblin.config.files.ambush, goblin.ambush).then(function(saved) {
            if (!saved) {
