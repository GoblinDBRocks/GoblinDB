...
function init() {
	Storage.read(goblin.config.files.db).then(function(db, success) {
		goblin.db = db;
...
function goblinChanged(details) {
	if (goblin.config.recordChanges) {
		...
		Storage.save(goblin.config.files.db, {}).then(function(success) {
			if (!success) {
				logger('DB_CLEAN_FS');
			}
			
			// And now, save the real db version
			Storage.save(goblin.config.files.db, goblin.db).then(function(success) {
				if (!success) {
					logger('DB_SAVE_FS');
				}
			});
