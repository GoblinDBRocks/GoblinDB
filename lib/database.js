...
goblin.config.watchForDatabaseChanges = true;

// Goblin Internal Events + Hooks Execution
goblin.goblinDataEmitter.on('change', function(details) {
	if (goblin.config.recordChanges && details.type !== 'external') {
		// Make chokidar ignore same goblin instance changes.
		goblin.config.watchForDatabaseChanges = false;
...
function init(cb) {
	Storage.read(goblin.config.files.db, {}, function(err, db) {
		...
		// Whatch for external changes to the database file.
		activateWatcher();
		cb();
	})
}
...
/**
 * Watch for changes on the current database file, if there's any change then updates
 * the database in memory (goblin.db) and trigger and "change" event with the "external"
 * type.
 *
 * The reason for time out is https://github.com/paulmillr/chokidar/issues/365. This is
 * not the best solution but it's the best one we've given thought so far.
 * 
 * @returns {void}
 */
function activateWatcher() {
	chokidar
		.watch('./' + goblin.config.files.db, {persistent: true})
		.on('change', (path, stats) => {
			setTimeout(() => {
				if (goblin.config.watchForDatabaseChanges) {
					// Read the file and update database.
					const oldDB = Object.assign({}, goblin.db);
	
					Storage.read(goblin.config.files.db, {}, (err, db) => {
						if (err) {
							return logger('DB_HAS_INVALID_DATA', err);
						}
